...
function Table<TableData extends TableRow>({ columns, data }: TableProps<TableData>) {

  return <table>
    <thead>
      <tr>
        {columns.map(column => <td className="p-2" key={column.key}>{column.title}</td>)}
      </tr>
    </thead>
    <tbody>
      {data.map(row => <tr key={row.id}>
        {columns.map(column => <td className="p-2" key={`${row.id}-${column.key}`}>{column.value(row)}</td>)}
      </tr>)}
    </tbody>
  </table>
}
...
export default function Route({ loaderData }: Route.ComponentProps) {
  return (
    <Center>
      ...
      <h1 className="text-lg flex justify-center">Usuários</h1>
      <Spacer size="md" />
      <Table
        data={loaderData.users}
        columns={[
          { key: "id", title: "Id", value: (user) => user.id },
          { key: "name", title: "Nome", value: (user) => <Link to={`/user/${user.id}`}>{user.name}</Link> },
          ...
          { key: "edit", title: "Ações", value: (user) => <LinkButton styleType="secondary" to={`/user/${user.id}/edit`}>Editar</LinkButton> }
        ]}
      />
    </Center>
  );
